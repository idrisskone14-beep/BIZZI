-- Bizzi V250 - annuaire prestataires pagine pour 100 000 profils.
-- A executer dans Supabase SQL Editor avant de publier la V250.
...
declare
  ...
  city_value text := lower(trim(coalesce(p_city, '')));
  service_value text := lower(trim(coalesce(p_service, '')));
...
begin
  ...
  with candidates as (
    ...
    where p.status = 'approved'
      ...
      and (
        city_value = ''
        or lower(coalesce(c.name, '')) = city_value
        or lower(coalesce(co.name, '')) = city_value
      )
      and (
        service_value = ''
        or lower(coalesce(p.requested_service_name, '')) = service_value
        or exists (
          select 1
          ...
          where service_filter.provider_id = p.id
            and lower(service_row.name) = service_value
        )
      )
      and (
      ...
      and (
        ...
        or lower(coalesce(p.description, '')) like '%' || search_value || '%'
        or lower(coalesce(p.neighborhood, '')) like '%' || search_value || '%'
        or lower(coalesce(c.name, '')) like '%' || search_value || '%'
        or lower(coalesce(co.name, '')) like '%' || search_value || '%'
        or lower(coalesce(p.requested_service_name, '')) like '%' || search_value || '%'
        or lower(coalesce(p.requested_category_name, '')) like '%' || search_value || '%'
        or (
        ...
        or exists (
          ...
          where search_link.provider_id = p.id
            and (
              lower(search_service.name) like '%' || search_value || '%'
              or lower(coalesce(search_category.name, '')) like '%' || search_value || '%'
            )
        )
      )
  ), page_with_extra as (
