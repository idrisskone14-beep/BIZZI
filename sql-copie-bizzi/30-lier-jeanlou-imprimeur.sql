-- Bizzi - Lier Jeanlou au service Imprimeur V73
-- Objectif : retrouver le prestataire Jeanlou, le lier a Imprimeur,
...
declare
  ...
  match_count integer;
begin
  -- Recherche robuste : Jeanlou, Jean Lou, JEAN-LOU, etc.
  select count(*)
  into match_count
  from public.providers p
  where regexp_replace(lower(coalesce(p.full_name, '')), '[^a-z0-9]+', '', 'g') like '%jeanlou%';

  if match_count = 0 then
    raise exception 'Aucun prestataire trouve avec le nom Jeanlou. Verifiez le nom exact dans providers.';
  end if;

  if match_count > 1 then
    raise exception 'Plusieurs prestataires Jeanlou trouves. Utilisez le numero de telephone pour eviter une mauvaise affectation.';
  end if;
...
-- Controle final : Jeanlou doit ressortir avec le service Imprimeur.
select
  p.id,
  p.full_name,
  p.phone,
