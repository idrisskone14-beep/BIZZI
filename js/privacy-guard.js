(function (root) {
  ...
  function paymentPanel({ method, amountText, account, accountReady }) {
    ...
    return `
      ...
      <p>${cash ? "Vous paierez en espèces au chauffeur ou au livreur à la fin de la prestation." : "Après paiement, Bizzi ouvre la demande aux livreurs proches. Le contact du livreur reste masqué jusqu'à acceptation."}</p>
      <div class="payment-account-card ${accountReady ? "ready" : "todo"}">
        <span>${cash ? "Paiement à la remise" : `Compte Bizzi ${escapeHtml(method)}`}</span>
        <strong>${escapeHtml(accountReady ? account : "Compte Bizzi à renseigner")}</strong>
      </div>
