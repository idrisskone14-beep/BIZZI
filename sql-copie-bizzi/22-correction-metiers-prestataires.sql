-- Bizzi - Correction metiers prestataires
-- Objectif : affecter les bons metiers a Jean Francois, Amir Bakayoko
-- et Test Minimal Bizzi, sans supprimer les autres donnees.
--
-- A executer dans Supabase > SQL Editor > New query.

insert into categories (name, sort_order)
values ('Maison & Travaux', 10)
on conflict (name) do update
set sort_order = excluded.sort_order,
    is_active = true;

insert into services (category_id, name, sort_order)
select c.id, 'Maçon', 110
from categories c
where c.name = 'Maison & Travaux'
