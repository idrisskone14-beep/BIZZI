-- Bizzi V110 - Rattrapage Josey / Jardinier Paysagiste
-- A executer dans Supabase SQL Editor uniquement si Josey reste bloque en pending - trial.
--
-- Ce script est idempotent :
-- - il ne supprime rien ;
-- - il cree le service "Jardinier / Paysagiste" si besoin ;
-- - il rattache Josey au bon metier ;
-- - il active le mois gratuit.
...
begin
  ...
  where lower(p.full_name) like '%josey%'
     or public.bizzi_phone_digits(p.phone) = public.bizzi_phone_digits('+2250200000002')
  order by p.created_at desc
  limit 1;

  if provider_uuid is null then
    raise exception 'Aucun prestataire Josey trouve. Verifiez le nom ou le numero dans providers.';
  end if;
...
select
  'rattrapage_josey_paysagiste_v110' as verification,
  p.id,
  p.full_name,
  p.phone,
  p.status,
...
where lower(full_name) like '%josey%'
   or public.bizzi_phone_digits(phone) = public.bizzi_phone_digits('+2250200000002');
