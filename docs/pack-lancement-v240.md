...
2. Envoyer le ZIP `BIZZI-V240-CLOUDFLARE-UPLOAD-ONLY.zip` sur Cloudflare Pages.
3. Purger le cache Cloudflare.
4. Ouvrir `https://bizzi-africa.com/?v=240&refresh=1` sur un téléphone de test.
5. Vérifier que l'interface et l'administration affichent V240.
6. Vérifier les migrations Supabase, fonctions backend et vues publiques nécessaires.
7. Déployer la fonction Supabase `voice-transcribe` V240 ; le ZIP Cloudflare ne déploie pas ce fichier serveur.
...
10. Exécuter tous les scénarios du document `tests-terrain-v240.md`.
11. Compléter la checklist de publication V240.
12. Ouvrir d'abord à un groupe pilote limité.

## Contrôles spécifiques V240
