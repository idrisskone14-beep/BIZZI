...
const LOCAL_DELIVERY_SERVICE = "Bizzi Livraison";
...
const seed = {
  ...
  categories: [
    ...
    },
    {
      ...
      services: ["Bizzi Livraison", "Chauffeur", "Livreur de gaz en bouteille", "Location de véhicules", "Mécanicien", "Remorquage / Dépannage auto", "Dépannage moto", "Vulcanisateur / Pneus", "Carrossier / Peintre auto", "Lavage auto / moto", "Transport de marchandises", "Transport de colis international", "Transitaire", "Conducteur moto-taxi"],
    },
    {
...
const SERVICE_ALIASES = {
  "Livraison": "Bizzi Livraison",
  "Livreur": "Bizzi Livraison",
  "Coursier": "Bizzi Livraison",
  "Service livraison": "Bizzi Livraison",
...
const SERVICE_KEY_ALIASES = {
  ...
  livreur: "Bizzi Livraison",
  coursier: "Bizzi Livraison",
  servicelivraison: "Bizzi Livraison",
...
function setProviderServiceSelect(serviceName = "Bizzi Livraison") {
...
function openCourierProfileCreation() {
  providerSignupSessionActive = true;
  const service = document.querySelector("#deliveryRequestType")?.value === "ride" ? "Chauffeur" : "Bizzi Livraison";
...
function openCourierRenewal() {
  ...
  state.selectedService = document.querySelector("#deliveryRequestType")?.value === "ride" ? "Chauffeur" : "Bizzi Livraison";
...
function assistantServiceHints() {
  return {
    ...
    "Bizzi Livraison": ["livraison", "livrer", "livreur", "coursier", "colis", "paquet", "course", "recuperer", "deposer", "expedier", "porter", "proche de moi", "pres de moi", "moto", "rapide"],
...
function selectDeliveryService({ clearSearch = true, resetFilters = false } = {}) {
  const delivery = allServices().find((item) => item.name === "Bizzi Livraison")
    || { name: "Bizzi Livraison", category: "Transports & Logistique" };
...
async function startCourierLiveAvailability(button = null) {
  ...
  if (!deliveryProviderSupportsAny(provider)) {
    renderProviderStatus("Ce profil n'est pas configuré pour les livraisons ou courses. Choisissez Bizzi Livraison, Chauffeur ou Conducteur moto-taxi.");
...
function renderProviders() {
  ...
  const secureDeliveryMode = state.selectedService === "Bizzi Livraison" && !query;
...
function profitableServices() {
  ...
  state.deliveryRequests
    .filter((request) => request.paymentStatus === "approved")
    .forEach((request) => byService.set("Bizzi Livraison", (byService.get("Bizzi Livraison") || 0) + Number(request.bizziCommission || 0)));
