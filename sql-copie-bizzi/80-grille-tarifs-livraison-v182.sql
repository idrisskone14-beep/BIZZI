-- Bizzi V182 - Grille transparente de tarification livraison
-- Objectif : aligner la base sur la nouvelle grille appliquee dans l'application.
-- Minimum livraison : 1 000 FCFA.
-- Commission Bizzi : 15%.
-- Urgence : +20%.
-- Heures de pointe : +20%.
-- Apres 12 km : +350 FCFA par km supplementaire.
...
begin
  ...
  if coalesce(delivery_record.amount, 0) < 1000 then
...
select
  'Bizzi V182 grille livraison transparente installee' as message,
  count(*) as demandes_livraison,
  count(*) filter (where amount >= 1000) as demandes_avec_minimum_ok,
