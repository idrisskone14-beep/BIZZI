# Sécurité et stabilité Bizzi V240
...
- Le modèle de transcription est configuré côté serveur et peut être remplacé par le secret `OPENAI_TRANSCRIBE_MODEL` sans exposer de clé.
- Le contexte envoyé par le navigateur est limité à la langue et à la ville courante ; le serveur nettoie et tronque cette valeur.
...
- Après l'envoi du ZIP, purger Cloudflare puis ouvrir `?v=240&refresh=1`.
...
## Accueil premium V240

- Refonte visuelle isolée de l’accueil, sans migration des données ni modification des clés de stockage.
- Commande vocale existante conservée et accessible directement depuis le hero.
- Composition graphique native CSS, sans image lourde supplémentaire.
- Mode connexion lente : maquettes décoratives retirées automatiquement.
- Navigation vers Services, Livraison, Food, Événements, Emplois et Prestataire conservée.

## Visuels de parcours V240

- Cinq images fournies sont intégrées avec chargement différé et décodage asynchrone.
- Les photos sont masquées en mode connexion lente pour préserver les données mobiles.
- Les formulaires, la recherche vocale et les clés de stockage restent inchangés.


## Onglets illustrés V240

- Le visuel Emplois est ajouté au parcours dédié.
- Les cinq onglets principaux utilisent leur image correspondante.
- Toutes les images sont différées et masquées en mode connexion lente.
- La recherche vocale et les données existantes restent inchangées.
